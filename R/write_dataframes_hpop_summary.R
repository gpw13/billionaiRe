...
write_baseline_projection_hpop_summary <- function(df,
    ...
    baseline_proj_header <- vec2emptyDF(c("Raw Value", rep("", length(value)*2-1), "",
    ...
    baseline_proj_subHeader <- vec2emptyDF(c(rep(start_end_years, length(value)), "",
...
write_billion_contribution_hpop_summary <- function(df,
  ...
  openxlsx::writeFormula(wb, sheet = sheet_name,
                        x = c(glue::glue('=SUMIF({contrib_thsd_col}{boxes_bounds$contribution["start_row"]+3}:{contrib_thsd_col}{boxes_bounds$contribution["end_row"]},">0")'),
