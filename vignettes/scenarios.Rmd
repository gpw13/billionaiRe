...
-   *Trajectory-based scenarios*: certain targets or trajectories must be reached by a certain date (e.g. Sustainable Development Goal targets (e.g. `sdg` scenarios), or `acceleration` scenarios from technical programs). Those scenarios should start from 2018 as the GPW13 baseline year.
...
The *event-based* and *trajectory-based* scenarios are storing values that are alternative, plausible, description of how the future may develop. The *internal use* scenarios hold values to highlight where the data comes from.
...
billionaiRe `transform_` and `calculate_` functions (as well as `rapporteur` export functions) use the `scenario` column as a `group` in [dplyr::group_by](https://dplyr.tidyverse.org/reference/group_by.html). *Scenarios should then contain all the required data needed for billionaiRe calculations*. If a scenario does not contain the required data, the functions will fail.
...
The definition of the *internal use* scenarios happen in the `ingestion` and `projecting` steps. 

The *event-based* and *trajectory-based* scenarios are calculated at stage 3 (Calculating), as does the rest of the billionaiRe calculations. Those scenarios can be calculated with the `add_scenario()` function.

Within the billionaiRe calculations pipeline, adding the scenarios values should ideally be the first steps. However, some  event-based and trajectory-based scenarios, might require some level of data recycling before they run. If this is the case, errors should be produced.
